  ...
  const stats = [
    ...
    { label: 'Number of files', value: d3.groups(data, d => d.file).length },
    {
      ...
      value: (() => {
        ...
        return d3.greatest(fileLengths, d => d[1])[1];
      })()
    },
...
function renderTooltipContent(commit) {
  ...
  document.getElementById('commit-date').textContent = commit.datetime?.toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' });
...
function drawScatter(data, commits) {
  ...
  const usableArea = { top: margin.top, right: width - margin.right, bottom: height - margin.bottom, left: margin.left, width: width - margin.left - margin.right, height: height - margin.top - margin.bottom };

  const svg = d3.select('#chart').append('svg').attr('width', width).attr('height', height);

  const yScale = d3.scaleLinear().domain([0, 24]).range([usableArea.bottom, usableArea.top]);
  const xExtent = d3.extent(data, d => d.date);
  const xScale = d3.scaleTime().domain([d3.timeDay.offset(xExtent[0], -1), d3.timeDay.offset(xExtent[1], 1)]).range([usableArea.left, usableArea.right]);
  ...
  const dots = svg.append('g')
    ...
    .on('mouseenter', (event, d) => { d3.select(event.currentTarget).style('fill-opacity', 1); renderTooltipContent(d); updateTooltipVisibility(true); updateTooltipPosition(event); })
    .on('mousemove', (event) => updateTooltipPosition(event))
    .on('mouseleave', (event) => { d3.select(event.currentTarget).style('fill-opacity', 0.7); updateTooltipVisibility(false); });
